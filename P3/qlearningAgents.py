...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            maxVal[action] = self.getQValue(state, action)

        return maxVal[maxVal.argMax()]
    ...
    def computeActionFromQValues(self, state):
        ...
        maxAction = util.Counter()
        if not len(self.getLegalActions(state)):
            return None
        for action in self.getLegalActions(state):
            maxAction[action] = self.getQValue(state, action)

        return maxAction.argMax()
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.values[(state, action)] = self.alpha * s + (1 - self.alpha) * qVal
